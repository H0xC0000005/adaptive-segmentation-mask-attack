...
class EyeDatasetTest(Dataset):
    def __init__(self, image_path, mask_path, in_size=572, out_size=388):
        super(EyeDatasetTest, self).__init__()
    ...
    def __getitem__(self, index):
        ...
        # Crop image
        # >>> need to change this hardcoded cropping part for cityscape dataset
        im_as_np = im_as_np[:, 70:70 + 408, 10:-10]  # 428 x 428 ? 408 x 408 for sample input G_5_R_1
        # Pad image, why pad with this size?
        pad_size = 82
        im_as_np = np.asarray([np.pad(single_slice, pad_size, mode='edge')  # pad fore- and afterward
                               for single_slice in im_as_np])  # 572x572 if original is 408x408
        """
        # Sanity check
