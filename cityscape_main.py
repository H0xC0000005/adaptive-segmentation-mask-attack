...
if __name__ == '__main__':
    ...
    model: nn.Module
    model = load_model('/home/peizhu/PycharmProjects/adaptive-segmentation-mask-attack/models'
    ...
    if isinstance(model, dict):
        print(f"len of model: {len(model)}")
        layerwise_len = ""
        for k in model.keys():
            elem = model[k]
            try:
                print(f"got type with len(): {type(elem)}")
